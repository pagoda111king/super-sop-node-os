...
| 层 | 中文名 | 一句话解释 | 典型输入/输出 |
|---|---|---|---|
| L1 | 输入捕获层 | 接住人、文件、消息、网页、会议、视频、表单、截图、语音、代码、数据库事件等原始输入 | 原始混沌输入 -> 清晰任务对象 |
| L2 | 上下文记忆层 | 把任务放回正确背景里，加载项目规则、历史经验、用户偏好、技能、权限、工作区边界 | 任务对象 -> 上下文包 |
| L3 | 证据数据层 | 把“我觉得”变成“有来源、有结构、能查询”的证据和数据 | 资料/事实 -> 证据集、结构化记录、schema |
| L4 | 编排学习层 | 决定怎么走、谁来做、怎么分支、需要先学什么 | 目标和证据 -> 计划、路线、学习材料 |
| L5 | 执行保障层 | 真的动手做，并用测试、评分、检查证明它做对了 | 计划 -> 产物、测试结果、评分、Check |
| L6 | 交付进化层 | 把结果交给人或系统，并把有效经验沉淀成下次可复用资产 | 产物 -> 交付包、规则、模板、记忆、SOP |

这一版的重点是：

```text
输入捕获层不是只接文字需求。
它要能接住现实世界里所有进入系统的原始信号。
```
...
这些都不是“已经干净的任务”，而是要先被 Intake 节点接住、澄清、归类、压缩成可处理任务。

### 14 类核心节点

| 节点 | 中文名 | 作用 |
|---|---|---|
| Intake | 输入捕获节点 | 接住人、文件、消息、网页、会议、视频等原始输入，把混乱信号变成清晰任务对象 |
